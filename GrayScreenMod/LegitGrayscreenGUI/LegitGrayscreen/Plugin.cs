...
namespace LegitGrayscreen
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        // GUI stuff
        private bool greyscreen = true;
        private float threshold = 0.32f;
        ...
        private void Update()
        {
            ...
            bool shouldSnap = (num > threshold || bothJoysticksPressed) && delay > 32;
            if (shouldSnap)
            {
                drivers = Object.FindObjectsOfType<TrackedPoseDriver>();
                snapped = true;
        ...
        private void OnGUI()
        {
            ...
            GUI.BeginGroup(new Rect(20, 20, 250, 100), "Grayscreen", GUI.skin.window);
            greyscreen = GUI.Toggle(new Rect(10, 25, 200, 20), greyscreen, "Enable GrayScreen");
            GUI.Label(new Rect(10, 50, 200, 20), $"Flick Threshold: {threshold:F2}");
            threshold = GUI.HorizontalSlider(new Rect(10, 70, 200, 20), threshold, 0.05f, 1f);
            GUI.EndGroup();
        }
    }
}
